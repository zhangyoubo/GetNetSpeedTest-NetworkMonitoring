...
+ (instancetype)resultSetWithStatement:(LYTStatement *)statement usingParentDatabase:(LYTDatabase*)aDB {
    ...
    return LYTDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    LYTDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return LYTDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"LYTDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
...
- (void)setParentDB:(LYTDatabase *)newDb {
    _parentDB = newDb;
}
