...
@interface LYTDatabasePool()

- (void)pushDatabaseBackInPool:(LYTDatabase*)db;
- (LYTDatabase*)db;

@end


@implementation LYTDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return LYTDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"LYTdb.%@", self] UTF8String], NULL);
        _databaseInPool     = LYTDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = LYTDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    LYTDBRelease(_path);
    LYTDBRelease(_databaseInPool);
    LYTDBRelease(_databaseOutPool);
    
    if (_lockQueue) {
        LYTDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(LYTDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The LYTDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (LYTDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [LYTDatabase databaseWithPath:self->_path];
