...
@implementation LYTNetTraceRoute

/**
 * 初始化
 */
- (LYTNetTraceRoute *)initWithMaxTTL:(int)ttl
                            timeout:(int)timeout
...
- (Boolean)doTraceRoute:(NSString *)host
{
    ...
    NSArray *serverDNSs = [LYTNetGetAddress getDNSsWithDormain:host];
    ...
    while (ttl <= maxTTL) {
        ...
        for (int try = 0; try < maxAttempts; try ++) {
            startTime = [LYTNetTimer getMicroSeconds];
            //发送成功返回值等于发送消息的长度
            ...
            if (FD_ISSET(recv_sock, &readfds) > 0) {
                ...
                if ((res = recvfrom(recv_sock, buf, 100, 0, (struct sockaddr *)&fromAddr, &n)) <
                    ...
                    icmp = true;
                    delta = [LYTNetTimer computeDurationSince:startTime];
